...
void PathPlanner::afm2(GridMap &gm, double heading, double theta, int radius) {
  auto dst = gm.destination();
  int row = dst.first;
  int col = dst.second;
  for(int i=row-radius; i<=row+radius; i++) {
    for(int j=col-radius; j<=col+radius; j++) {
      GridCell *curCell = gm.getCellByRowCol(i, j);
      if (curCell == nullptr || (i==row && j==col)) {
        // 跳过空值和自身
        continue;
      }
      // liu yuanchang
      double distance = std::sqrt((i-row)*(i-row) + (j-col)*(j-col));
      double angle = std::atan2(j-col, row-i)*180/M_PI;
      if (distance <= radius && std::abs(angle-heading) > theta) {
        curCell->setVelocity(0);
      } else {
        // printf("feasible : (%d, %d), angle = %.3f\n", i, j, angle);
      }
    }
  }
  squaredXXX(gm, fmm);
...
void PathPlanner::my_afm2(GridMap &gm, double heading, int radius) {
  auto dst = gm.destination();
  int row = dst.first;
  int col = dst.second;
  // 左舷圆心
  double core_row = row - 1.0*radius*std::sin(heading/180*M_PI);
  double core_col = col - 1.0*radius*std::cos(heading/180*M_PI);
  auto left = std::make_pair(std::round(core_row), std::round(core_col));
  // 右舷圆心
  auto right = std::make_pair(std::round(2*row - core_row), std::round(2*col - core_col));
  ...
  for(int i=row-2*radius; i<=row+2*radius; i++) {
    for(int j=col-2*radius; j<=col+2*radius; j++) {
      ...
      // yy
      double distance_left = std::sqrt((i-left.first)*(i-left.first) + (j-left.second)*(j-left.second));
      double distance_right = std::sqrt((i-right.first)*(i-right.first) + (j-right.second)*(j-right.second));

      // 此处的ceil非常关键，不加可能出现不可行区域堵死船舶
      if (std::ceil(distance_left) < radius || std::ceil(distance_right) < radius) {
        curCell->setVelocity(0);
      }
